...
Legacy shell runners are deprecated. Do not use `run_matrix.sh`, `calibrate.sh`, or `run_single.sh` for the new dataset. Session `20260921-202841` is complete; its reviewed findings are in `processed/review/interpretation.md` inside that session and in `paper/paper_draft.md`. The old paper is preserved in `paper/archive/paper_draft_legacy.md`; its empirical claims and the legacy figures should not be reused as completed-matrix evidence.
