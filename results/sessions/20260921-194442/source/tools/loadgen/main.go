...
func main() {
	...
	if *pattern == "bursty" { *rate = (*burstLow + *burstHigh) / 2 }
		...
		go func() {
			...
			for task := range taskChan {
				...
				if err != nil {
					...
					resp.Body.Close()
					if bodyErr != nil { res.Error = bodyErr.Error() }
				}
				reqEnd := time.Now()
	...
	for {
		...
		// Calculate next scheduled arrival deadline
		var nextInterval time.Duration
		if *pattern == "bursty" {
			elapsed := currentScheduledTime.Sub(testStartTime)
			cyclePos := elapsed % *burstPeriod
			if cyclePos < halfBurst {
				nextInterval = time.Duration(float64(time.Second) / *burstLow)
			} else {
				nextInterval = time.Duration(float64(time.Second) / *burstHigh)
			}
		} else {
			nextInterval = time.Duration(float64(time.Second) / *rate)
		}

		currentScheduledTime = currentScheduledTime.Add(nextInterval)
	}

	if remaining := time.Until(testEndTime); remaining > 0 { time.Sleep(remaining) }
