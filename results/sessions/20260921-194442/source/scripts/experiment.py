...
def cooldown(limit=65, timeout=300, minimum=5):
    time.sleep(minimum)
    deadline = time.monotonic() + timeout
    while True:
        vals = temperatures()
        if not vals:
            raise RuntimeError('No readable temperature sensor; investigate before formal measurements.')
        if max(vals.values()) <= limit:
            return vals
        if time.monotonic() >= deadline:
            raise RuntimeError('Temperature did not fall below 65 C within five minutes.')
        print('Cooling: ', vals, flush=True)
        time.sleep(5)


def provenance():
    files = list((ROOT / 'service').rglob('*.go')) + list((ROOT / 'tools/loadgen').glob('*.go'))
    files += list((ROOT / 'scripts').glob('*.py')) + list((ROOT / 'analysis').glob('*v2.py')) + [ROOT / 'service/Dockerfile', LOADGEN]
...
def loadgen_args(config, seconds, output):
    args = ['taskset', '-c', CLIENT_CPUS, LOADGEN, '-url',
            ...
            '-rate', str(config['rate']), '-concurrency', '128', '-output', output]
...
def trial(session, phase, run_id, config, duration=60, warmup=20):
    ...
    try:
        ...
        report = {
            ...
            'timestamp_start': started, 'timestamp_end': time.time(), 'config': config,
...
def calibrate(session):
    ...
    for idx, multiplier in enumerate([.5, .7, .85, 1.0, 1.15, 1.35, 1.6, 2.0]):
        ...
        if set(r['quality_flags']) & {'scheduler_late', 'insufficient_monitor_samples'}:
...
def preflight(session, calibration):
    ...
    for period in [10000, 50000, 100000, 250000]:
        ...
        passed = .45 <= usage <= .55 and not (set(r['quality_flags']) & {'scheduler_late', 'insufficient_monitor_samples'})
